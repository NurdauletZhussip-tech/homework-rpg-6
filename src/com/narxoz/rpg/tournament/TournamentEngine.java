...
public class TournamentEngine {
    ...
    public TournamentResult runTournament() {
        ...
        result.addLine("TOURNAMENT STARTED");
        result.addLine(hero.getName() + " versus " + opponent.getName());
        ...
        while (hero.isAlive() && opponent.isAlive() && round < maxRounds) {
            ...
            if (hero.getHealPotions() > 0) {
                actionQueue.enqueue(new HealCommand(hero, 40));
            }
            actionQueue.enqueue(new DefendCommand(hero, 0.20));
            ...
            result.addLine(opponent.getName() + " attacks for " + opponentDamage + " dmg");
            defenseChain.handle(opponentDamage, hero);

            result.addLine("after defense: Hero HP = " + hero.getHealth());

            if (!hero.isAlive()) {
                result.addLine(hero.getName() + "defeated!");
                break;
            }
        }
